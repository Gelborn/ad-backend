-- 0017_fix_release_donation_partnered_items.sql
...
begin
  ...
  -- 2) Seleciona primeiro pacote em estoque,
  --    juntando packages → items para filtrar pelo restaurante
  select
    p.id
  into v_pkg
    from packages p
  ...
  -- 4) Decrementa estoque do pacote
  update packages
     set stock = stock - 1
   where id = v_pkg.id;
