-- 0042_v_admin_donations_ongoing.sql
-- Ongoing donations (pending | accepted) with nested details.
-- Now includes OSC info inside each donation_intent.
...
SELECT
  ...
  /* donation_intents (array) — now with nested osc (id,name) */
  (
    SELECT COALESCE(
      jsonb_agg(
        jsonb_build_object(
    ...
    JOIN public.osc oi ON oi.id = di.osc_id
