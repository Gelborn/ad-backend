-- 20250806_release_donation_partnered_fix_returning.sql
...
BEGIN
  ...
  -- 2) Agrupa todos os pacotes in_stock
  ...
  -- 4) Marca todos os pacotes como awaiting_acceptance
  UPDATE packages
     SET status = 'awaiting_acceptance'
   WHERE id = ANY(v_pkgs);

  -- 5) Insere a doação e usa v_code no RETURNING
  ...
  RETURNING id, v_code  -- <-- aqui, pegamos a variável, não a coluna
    INTO donation_id, security_code;

  -- 6) Vínculo dos pacotes
