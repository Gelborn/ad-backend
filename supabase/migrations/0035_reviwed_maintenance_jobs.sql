...
begin
  ...
  raise notice 'Cron expire-and-reroute-intents ran successfully: % intents processed.', v_count;
  return v_count;
end;
$$;
...
begin
  ...
  if donations_denied = 0 then
    packages_released := 0;
    raise notice 'Cron deny-pickup-deadlines ran successfully: 0 donations denied, 0 packages released.';
    return next;
    return;
  end if;
  ...
  raise notice 'Cron deny-pickup-deadlines ran successfully: % donations denied, % packages released.', donations_denied, packages_released;
  return next;
end;
$$;
...
begin
  ...
  raise notice 'Cron discard-expired-packages ran successfully: % packages discarded.', v_count;
  return v_count;
end;
$$;
...
-- Set up cron jobs to run every 10 minutes
do $$
declare
  has_cron boolean;
  v_jobid int;
begin
  ...
  -- Helper to drop by job name if exists
  -- expire-and-reroute-intents
  select jobid into v_jobid from cron.job where jobname = 'expire-and-reroute-intents';
  if v_jobid is not null then
    perform cron.unschedule(v_jobid);
    raise notice 'Dropped existing expire-and-reroute-intents job.';
  end if;
  ...
  raise notice 'Created cron job expire-and-reroute-intents (*/10).';

  -- deny-pickup-deadlines
  select jobid into v_jobid from cron.job where jobname = 'deny-pickup-deadlines';
  if v_jobid is not null then
    perform cron.unschedule(v_jobid);
    raise notice 'Dropped existing deny-pickup-deadlines job.';
  end if;
  perform cron.schedule(
    'deny-pickup-deadlines',
    '*/10 * * * *',
  ...
  raise notice 'Created cron job deny-pickup-deadlines (*/10).';

  -- discard-expired-packages
  select jobid into v_jobid from cron.job where jobname = 'discard-expired-packages';
  if v_jobid is not null then
    perform cron.unschedule(v_jobid);
    raise notice 'Dropped existing discard-expired-packages job.';
  end if;
  perform cron.schedule(
    'discard-expired-packages',
    '*/10 * * * *',
  ...
  raise notice 'Created cron job discard-expired-packages (*/10).';
end
$$;
