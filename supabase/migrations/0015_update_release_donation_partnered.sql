-- arquivo de migration: 0015_update_release_donation_partnered.sql
...
begin
  ...
  -- 2. Pega primeiro pacote com stock > 0
  select id, security_code_template
    into v_pkg
    from packages
  ...
  -- 3. Busca parceria ativa, dando prioridade à favorita,
  --    e entre as demais escolhe a que recebeu há mais tempo
  with last_don as (
    select
      p.osc_id,
      p.is_favorite,
  ...
  select osc_id, is_favorite, last_received
    into v_partnership
    from last_don
   order by
