...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Required fields
    if (!name || !emailLc || !cep || !number || !street || !city || !uf) {
      return new Response(
    ...
    try {
      ...
      return new Response(
        ...
        { status: valErr.status ?? 422, headers: { ...corsHeaders(origin), "Content-Type": "application/json" } }
      );
    }

    /* ─── Duplicates ─── */
    const dupChecks = [
      { field: "email", value: emailLc, code: "EMAIL_ALREADY_EXISTS", msg: "Restaurante com esse e-mail já existe" },
      { field: "cnpj", value: cnpj, code: "CNPJ_ALREADY_EXISTS", msg: "Já existe restaurante com este CNPJ" },
      { field: "code", value: code, code: "CODE_ALREADY_EXISTS", msg: "Já existe restaurante com este código" },
    ];
    for (const check of dupChecks) {
      ...
      if (dupErr) {
        return new Response(
          JSON.stringify({ code: `${check.field.toUpperCase()}_CHECK_ERROR`, message: dupErr.message }),
          { status: 500, headers: { ...corsHeaders(origin), "Content-Type": "application/json" } }
        );
      }
      if (existing) {
        return new Response(
          JSON.stringify({ code: check.code, message: check.msg }),
    ...
    let geo;
    try {
      const gInput: GeocodeInput = { cep: cepDigits, number, street, city, uf };
      geo = await geocodeByCep(gInput);
    } catch (e: any) {
      return new Response(
        JSON.stringify({ code: "GEOCODE_ERROR", message: e?.message ?? "Falha ao geocodificar" }),
        { status: 422, headers: { ...corsHeaders(origin), "Content-Type": "application/json" } }
      );
    }
    ...
    if (userErr) {
      return new Response(
        ...
        { status: userErr.status ?? 400, headers: { ...corsHeaders(origin), "Content-Type": "application/json" } }
      );
    }
    ...
    return new Response(
      ...
      { status: err?.status ?? 500, headers: { ...corsHeaders(origin), "Content-Type": "application/json" } }
    );
  }
};
