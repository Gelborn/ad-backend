...
/*  CLIENTS                                                           */
/* ------------------------------------------------------------------ */
const supaUser  = createClient(                     // para tudo que respeita RLS
  Deno.env.get("SUPABASE_URL")!,
  Deno.env.get("SUPABASE_ANON_KEY")!
);
const supaAdmin = createClient(                     // uso pontual p/ notificação
  Deno.env.get("SUPABASE_URL")!,
...
serve(async (req) => {
  ...
  try {
    ...
    /* --- RPC release_donation_safe (SECURITY DEFINER) -------------- */
    ...
    if (error) {
      ...
      if (found) {
        return new Response(JSON.stringify({ code: error.message, message: found.msg }), {
          status: found.status,
    ...
    return new Response(JSON.stringify({ donation_id, security_code }), {
      status: 200,
      headers: { ...corsHeaders(req.headers.get("origin")), "Content-Type": "application/json" },
    });

  } catch (err: any) {
    console.error("‼ restaurant_liberate_donation ERROR:", err);
    return new Response(
