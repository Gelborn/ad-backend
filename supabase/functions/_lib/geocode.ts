...
interface ViaCepResp { logradouro: string; bairro: string; localidade: string; uf: string; erro?: boolean; }
...
export async function geocodeByCep(cep: string, number?: string) {
  ...
  const n = await fetch(`${NOMINATIM_URL}?q=${query}&format=json&limit=1`);
